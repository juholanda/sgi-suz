...
export default function AlcadasPage() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      setAlcadas(alcadasData); setPlantas(plantasData); setClasses(classesData); setUsuarios(usuariosData); setLoading(false)
  ...
  const openEdit = (a: Alcada) => { setForm({ plantaId: a.plantaId, classeId: a.classeId, nivel: String(a.nivel), userId: a.userId }); setFormError(''); setModal({ open: true, editing: a }) }
  ...
  const handleSave = async () => {
    ...
    if (!form.userId) { setFormError('Selecione um aprovador'); return }
    setSaving(true); setFormError('')
    try {
      const res = await fetch(modal.editing ? `/api/backoffice/alcadas/${modal.editing.id}` : '/api/backoffice/alcadas', {
        method: modal.editing ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, nivel: Number(form.nivel) }),
      })
  ...
  const handleDelete = async (a: Alcada) => {
    if (!window.confirm(`Remover alçada: ${a.user.nome} como nível ${a.nivel} em ${a.planta.nome} / Classe ${a.classe.numero}?`)) return
    await fetch(`/api/backoffice/alcadas/${a.id}`, { method: 'DELETE' }); loadData()
  }

  const grouped = useMemo(() => {
    const g: Record<string, { planta: string; plantaId: string; classe: Alcada['classe']; classeId: string; niveis: Alcada[] }> = {}
    for (const a of alcadas) {
      if (fPlanta !== 'all' && a.plantaId !== fPlanta) continue
      if (fClasse !== 'all' && a.classeId !== fClasse) continue
      const key = `${a.plantaId}__${a.classeId}`
      if (!g[key]) g[key] = { planta: a.planta.nome, plantaId: a.plantaId, classe: a.classe, classeId: a.classeId, niveis: [] }
      g[key].niveis.push(a)
    }
    return Object.values(g).sort((a, b) => {
      const p = a.planta.localeCompare(b.planta)
      return p !== 0 ? p : a.classe.numero - b.classe.numero
    })
  }, [alcadas, fPlanta, fClasse])

  const hasFilter = fPlanta !== 'all' || fClasse !== 'all'
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        ...
        <button onClick={openCreate} style={{ padding: '8px 16px', fontSize: '14px', fontWeight: 500, color: 'white', background: '#0038A8', borderRadius: '4px', border: 'none', cursor: 'pointer' }}>+ Configurar Alçada</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end', paddingBottom: '12px' }}>
        ...
        <div>
          <div style={{ fontSize: '11px', fontWeight: 600, color: '#6B7280', textTransform: 'uppercase', marginBottom: '4px', letterSpacing: '0.05em' }}>Classe</div>
          <select value={fClasse} onChange={e => setFClasse(e.target.value)} className="field-input" style={{ width: '180px' }}>
            <option value="all">Todas as classes</option>
            {classes.sort((a, b) => a.numero - b.numero).map(c => <option key={c.id} value={c.id}>Classe {c.numero} — {c.descricao}</option>)}
          </select>
        </div>
        {hasFilter && (
          <button onClick={() => { setFPlanta('all'); setFClasse('all') }} style={{ alignSelf: 'flex-end', padding: '7px 12px', fontSize: '13px', color: '#475569', background: 'white', border: '1px solid #E2E8F0', borderRadius: '4px', cursor: 'pointer' }}>Limpar</button>
        )}
        <div style={{ alignSelf: 'flex-end', marginLeft: 'auto' }}>
          <span style={{ fontSize: '12px', color: '#6B7280' }}>{grouped.length} grupo(s) · {alcadas.length} alçada(s)</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '4px', textAlign: 'center', padding: '48px' }}>
          <p style={{ fontSize: '14px', color: '#94A3B8', margin: '0 0 8px 0' }}>Nenhuma alçada encontrada.</p>
          <p style={{ fontSize: '12px', color: '#94A3B8', margin: 0 }}>{hasFilter ? 'Tente ajustar os filtros.' : 'Configure as alçadas para que as aprovações funcionem corretamente.'}</p>
        </div>
      ) : (
            ...
            return (
              <div key={i} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '4px', padding: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                  <div style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 700, background: colors.bg, color: colors.text, border: `2px solid ${colors.border}`, borderRadius: '4px' }}>{g.classe.numero}</div>
                  <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                  {g.niveis.sort((a, b) => a.nivel - b.nivel).map(n => (
                    <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', border: `1px solid ${colors.border}`, background: colors.bg, borderRadius: '4px' }}>
                      <span style={{ width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, background: colors.text, color: 'white', borderRadius: '50%' }}>{n.nivel}</span>
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 500, color: colors.text }}>{n.user.nome}</div>
                        <div style={{ fontSize: '11px', color: colors.text, opacity: 0.7 }}>{n.user.matricula}</div>
                      </div>
      ...
      {modal.open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: 'white', borderRadius: '4px', padding: '24px', width: '480px', maxWidth: '90vw' }}>
            <h2 style={{ fontSize: '16px', fontWeight: 700, color: '#0F172A', margin: '0 0 20px 0' }}>{modal.editing ? 'Editar Alçada' : 'Configurar Alçada'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              <div>
                ...
                <select value={form.classeId} onChange={e => setForm(f => ({ ...f, classeId: e.target.value }))} className="field-input">
              ...
              <div>
                ...
                <select value={form.nivel} onChange={e => setForm(f => ({ ...f, nivel: e.target.value }))} className="field-input">
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <select value={form.userId} onChange={e => setForm(f => ({ ...f, userId: e.target.value }))} className="field-input">
